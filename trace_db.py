...
def get_request(trace_id: str) -> Optional[Dict[str, Any]]:
    c = _conn()
    row = c.execute("SELECT * FROM requests WHERE trace_id = ?", (trace_id,)).fetchone()
    if not row:
        return None
...
def get_tool_event(event_id: str) -> Optional[Dict[str, Any]]:
    c = _conn()
    row = c.execute("SELECT * FROM tool_events WHERE event_id = ?", (event_id,)).fetchone()
    if not row:
        return None
...
def get_agent_call(agent_call_id: str) -> Optional[Dict[str, Any]]:
    c = _conn()
    row = c.execute("SELECT * FROM agent_calls WHERE agent_call_id = ?", (agent_call_id,)).fetchone()
    if not row:
        return None
...
def snapshot_stats() -> Dict[str, Any]:
    ...
    stats["session_count"] = c.execute("SELECT COUNT(*) FROM sessions").fetchone()[0]
    stats["request_count"] = c.execute("SELECT COUNT(*) FROM requests").fetchone()[0]
    stats["message_request_count"] = c.execute(
        "SELECT COUNT(*) FROM requests WHERE api='messages'"
    ).fetchone()[0]
    stats["count_tokens_request_count"] = c.execute(
        "SELECT COUNT(*) FROM requests WHERE api='count_tokens'"
    ).fetchone()[0]
    stats["main_request_count"] = c.execute(
        "SELECT COUNT(*) FROM requests WHERE role_kind='main'"
    ).fetchone()[0]
    stats["subagent_request_count"] = c.execute(
        "SELECT COUNT(*) FROM requests WHERE role_kind='subagent'"
    ).fetchone()[0]
    stats["titler_request_count"] = c.execute(
        "SELECT COUNT(*) FROM requests WHERE role_kind='titler'"
    ).fetchone()[0]
    stats["external_request_count"] = c.execute(
        "SELECT COUNT(*) FROM requests WHERE role_kind='external'"
    ).fetchone()[0]
    stats["agent_call_count"] = c.execute("SELECT COUNT(*) FROM agent_calls").fetchone()[0]
    stats["open_agent_call_count"] = c.execute(
        "SELECT COUNT(*) FROM agent_calls WHERE status != 'completed'"
    ).fetchone()[0]
    stats["tool_event_count"] = c.execute(
        "SELECT COUNT(*) FROM tool_events WHERE event_type IN ('tool_use_response','tool_result')"
    ).fetchone()[0]
    stats["history_link_count"] = c.execute(
